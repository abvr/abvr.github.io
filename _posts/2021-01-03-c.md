...
This changes for objects with high densities such as neutron stars and black holes. If you've watched the film 'Interstellar', the techniques used to pull of flybys are still conditional and require a binary star system that are orbiting each other, and that the spacecraft which you're in happening to traverse from source to destination. Kip Thorne theorises the slingshot around the black hole, through which the craft could attain enough kinetic energy even which cannot said to be close to ~ 0.5c. This complicates issues further as we delve into general relativity, Schwarzschild geodesics, and the remote risks of falling into the event horizon beyond which, realistically, would end up in the singularity.  
