...
And from $$v^2 = v_{esc}}^{2}+v_{\infty }}^{2} $$ we get orbital velocity ($$v$$), local escape velocity ($$v_{esc}$$), and hyperbolic excess velocity $$ v_{\infty}$$), even a relatively small $$\Delta {v}$$ above $${v_{esc}}$$ yields us high excess speed of $$v_{\infty}$$ which is the Oberth manoeuvre that uses additional acceleration by falling into the gravitational well of the body (for heavier object such as black holes this would employ general relativity where the gravitational potential would change to a metric tensor). So unfortunately for non black hole like bodies such as planets, this wouldn't result in much orbital trajectory deviation and nil transfer of momentum and velocity gained will be mediocre, though this changes for objects with high densities such as neutron stars and black holes.
